 ...
 */
export function collectPluginMap<T>(
    plugins: MarkdownPlugin[],
    getter: (p: MarkdownPlugin) => Record<string, T> | undefined,
): Map<string, T> {
    const map = new Map<string, T>();
    for (const plugin of plugins) {
        const rules = getter(plugin);
        if (rules) {
 ...
 */
export function collectPluginHandlers<T>(
    plugins: MarkdownPlugin[],
    getter: (p: MarkdownPlugin) => T[] | undefined,
): T[] {
    const result: T[] = [];
    for (const plugin of plugins) {
        const handlers = getter(plugin);
        if (handlers) {
